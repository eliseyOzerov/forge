...
public enum Statistics {
    ...
    public static func mean(_ values: [CGFloat]) -> CGFloat {
        guard !values.isEmpty else { return 0 }
        return values.reduce(0, +) / CGFloat(values.count)
    }

    public static func variance(_ values: [CGFloat]) -> CGFloat {
        guard values.count > 1 else { return 0 }
        let avg = mean(values)
        return values.reduce(0) { $0 + ($1 - avg) * ($1 - avg) } / CGFloat(values.count)
    }

    public static func standardDeviation(_ values: [CGFloat]) -> CGFloat {
    ...
    public static func range(_ values: [CGFloat]) -> CGFloat {
    ...
    public static func normalizeMinMax(_ values: [CGFloat]) -> [CGFloat] {
        guard let lo = values.min(), let hi = values.max(), hi > lo else {
            return [CGFloat](repeating: 0, count: values.count)
        }
        let range = hi - lo
    ...
    public static func normalizeZScore(_ values: [CGFloat]) -> [CGFloat] {
        let avg = mean(values)
        let std = standardDeviation(values)
        guard std > 0 else { return [CGFloat](repeating: 0, count: values.count) }
    ...
    public static func mean<V: Vector>(_ vectors: [V]) -> V {
        ...
        var sums = [CGFloat](repeating: 0, count: dim)
        for v in vectors {
            for (i, c) in v.components.enumerated() { sums[i] += c }
        }
        let n = CGFloat(vectors.count)
    ...
    public static func standardDeviation<V: Vector>(_ vectors: [V]) -> V {
        guard vectors.count > 1 else { return V(components: [CGFloat](repeating: 0, count: vectors.first?.count ?? 0)) }
        let avg = mean(vectors)
        let dim = avg.count
        var variances = [CGFloat](repeating: 0, count: dim)
        for v in vectors {
